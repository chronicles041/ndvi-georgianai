...
const Report: React.FC = () => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      setTimeout(() => {
        const randomDisease = diseases[diseaseIndex];
        console.log("Selected Disease:", randomDisease);

        handleClassify(randomDisease);

        setDiseaseIndex((prevIndex) => (prevIndex + 1) % diseases.length);
      }, 3000);
    }
  };
  ...
  <PageLayout>
    ...
    <div className="flex-grow overflow-y-scroll p-4 bg-gray-50">
